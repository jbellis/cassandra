 ...
 */
public class CommitLogSegment2
{
    private static final Logger logger = LoggerFactory.getLogger(CommitLogSegment2.class);
...
//    private final MappedByteBuffer buffer;
     ...
     */
    public static CommitLogSegment2 freshSegment()
    {
        return new CommitLogSegment2(null);
    }
     ...
     */
    CommitLogSegment2(String filePath)
    {
        id = getNextId();
...
//            logFileAccessor = new RandomAccessFile(logFile, "rw");
            ...
            limit = DatabaseDescriptor.getCommitLogSegmentSize();
...
//            buffer = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(logFile)));
...
//            bufferStream = new DataOutputStream(new ChecksummedOutputStream(new ByteBufferOutputStream(buffer), checksum));
            DataOutputStream tmp = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(logFile)));
            tmp.writeInt(CommitLog.END_OF_SEGMENT_MARKER);
            tmp.close();
            FileOutputStream fos = new FileOutputStream(logFile, false);
            bufferStream = new DataOutputStream(new BufferedOutputStream(fos));
            bufferStream.writeInt(CommitLog.END_OF_SEGMENT_MARKER);
...
//            buffer.position(0);
     ...
     */
    public CommitLogSegment2 recycle()
    {
        // writes an end-of-segment marker at the very beginning of the file and closes it
//        DataOutputStream tmp = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(logFile, false)));
//        tmp.writeInt(CommitLog.END_OF_SEGMENT_MARKER);
//        tmp.close();
        ...
        return new CommitLogSegment2(getPath());
    }
    ...
    {
        return size <= (limit - bufferStream.size());
    }
   ...
   /**
    ...
    public ReplayPosition write(RowMutation mutation) throws IOException
    {
        ...
        bufferStream.writeLong(checksum.getValue());
...
//        }
    ...
    {
        return new ReplayPosition(id, bufferStream.size());
    }
