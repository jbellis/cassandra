...
public class Memtable
{
    ...
    private final ConcurrentNavigableMap<RowPosition, AtomicSortedArrayColumns> rows = new ConcurrentSkipListMap<>();
    ...
    */
    ...
    {
        AtomicSortedArrayColumns previous = rows.get(key);

        if (previous == null)
        {
            AtomicSortedArrayColumns empty = cf.cloneMeShallow(AtomicSortedArrayColumns.factory, false);
    ...
    public String contents()
    {
        ...
        builder.append("{");
        for (Map.Entry<RowPosition, AtomicSortedArrayColumns> entry : rows.entrySet())
        {
     ...
     */
    public Iterator<Map.Entry<DecoratedKey, AtomicSortedArrayColumns>> getEntryIterator(final RowPosition startWith, final RowPosition stopAt)
    {
        return new Iterator<Map.Entry<DecoratedKey, AtomicSortedArrayColumns>>()
        {
            private Iterator<Map.Entry<RowPosition, AtomicSortedArrayColumns>> iter = stopAt.isMinimum(cfs.partitioner)
                                                                               ? rows.tailMap(startWith).entrySet().iterator()
                                                                               : rows.subMap(startWith, true, stopAt, true).entrySet().iterator();
            private Map.Entry<RowPosition, AtomicSortedArrayColumns> currentEntry;
            ...
            public Map.Entry<DecoratedKey, AtomicSortedArrayColumns> next()
            {
                Map.Entry<RowPosition, AtomicSortedArrayColumns> entry = iter.next();
                ...
                return (Map.Entry<DecoratedKey, AtomicSortedArrayColumns>) (Object)entry;
            }
    ...
    private static class MeteringRunnable implements Runnable
    {
        ...
        public void run()
        {
            try
            {
                ...
                int objects = 0;
                for (Map.Entry<RowPosition, AtomicSortedArrayColumns> entry : memtable.rows.entrySet())
                {
